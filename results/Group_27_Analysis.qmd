...
Select the raw data, remove the NA value and extra commas from the data, then determine the corresponding data type. Among all variables, only Age and Hours_PW are numerical variables. The integrated data is output to 'cleaned_dataset27' for preliminary judgment of data and variables. 
...
By visualizing all the variables first, we can see that there are some variables with perfectly linear relationships among the classes (for example, Preschool in Education and Priv-house-serv in Occupation). 
...
In order to avoid affecting the model fitting results later, we sorted and merged some data in 'cleaned_dataset27_1.1'. 
...
# save data
write.csv(clean_data, "..\\datasets\\cleaned_dataset27_1.1.csv", row.names = FALSE)
```
...
White-collar and sales jobs have a higher percentage of individuals earning \>50K compared to blue-collar, service, and farming-fishing occupations. This suggests that occupation type plays a crucial role in income level.

-   **Income vs. Sex**

A higher percentage of males earn more than 50K compared to females. The income gap suggests that there might be gender differences in income.

-   **Income vs. Marital Status**

Married individuals have a significantly higher proportion of high-income earners compared to those who are never married or divorced/separated/widowed. This could indicate that marital stability is associated with higher earnings.
...
The residual deviation is reduced by 588.2 (1556.3→968.1), indicating that the modelreduces 38% of residual deviance compared to the null model. AIC=996.1 is significantly lower than the original model's AIC=998.68, indicating that the op
...
1.  Age
    ...
    **Age 0.025275 p=0.000689 \*\*\***
    ...
    Practical significance: For each year of age increase, the odds of earning \>50k increases by 2.5% (OR=1.025).
...
2.  Education level
    ...
    ***EducationHigh School -1.077184 p=0.000653***
    ...
    Those with a basic education degree are 93% less likely to have a high income than those with an advanced degree (OR=0.07) Those with a high school degree are 66% less likely to earn a high income than those with an advanced degree (OR=0.34)

3.  Marital status

    Marital_StatusMarried 2.626701 p\<2e-16 \*\*\* Married people are 13.8 times more likely to have a high income than the baseline group (OR=exp(2.626)=13.8)
...
Working 10 more hours per week increases the likelihood of high income by 52% (OR=1.52)
